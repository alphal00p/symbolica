...
mod test {
    ...
    fn nested_closed_sums_reuse_dummy_indices() {
        ...
        for expression in [
            parse!(
                "2*(a+b)*(c+d)*((f1(nested_counter_mu)+f2(nested_counter_mu))*(g1(nested_counter_mu)+g2(nested_counter_mu))+(h1(nested_counter_mu)+h2(nested_counter_mu))*(j1(nested_counter_mu)+j2(nested_counter_mu)))"
            ),
            parse!(
    ...
    fn sum_contractions_preserve_shared_open_indices() {
        ...
        for (expression, dummy_count, sum_factors) in [
            (
                parse!(
                    "2*(a+b)*(c+d)*((f1(mu1,mu2,counter_external)+f2(mu1,mu2,counter_external))*(g1(mu1,mu2)+g2(mu1,mu2))+(h1(mu1,mu2,counter_external)+h2(mu1,mu2,counter_external))*(j1(mu2,mu1)+j2(mu2,mu1)))"
                ),
                2,
                3,
            ),
            (
                parse!(
                    "2*(a+b)*(c+d)*(f1(mu1,mu2,counter_external)+f2(mu1,mu2,counter_external))*(g1(mu1,mu3)+g2(mu1,mu3))*(h1(mu2,mu3)+h2(mu2,mu3))"
                ),
                3,
                5,
            ),
